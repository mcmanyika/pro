...
def pledge(request, id):
    ...
    c = connection.cursor()
    c.cursor.execute("""Select d.name
...
def member_details(request, id):
    ...
    t.cursor.execute("""Select
                            ...
                            FROM libs_t_payment p
                            Where p.rootid = %s and p.commitment = 'Cash'""", [id])
    ...
    pt.cursor.execute("""Select
                            sum(p.amount) as amount
                            FROM libs_t_payment p
                            Where p.rootid = %s AND p.commitment = 'Pledge'""", [id])
...
def children(request):
    ...
    row.cursor.execute("""Select a.id, a.fname, a.lname, a.gender, a.image, ap.id as parent_id, 
